...
for i=firstimage:(r)               % run through all images
    ...
    input = uint8(mean(double(imread(filenamelist((i),:))),3));       % read in the image which has to be correlated
    ...
    imshow(filenamelist(i,:))                     % update image
    hold on
    ...
    estimatedtime=sum(time)/i*(r);            % estimate time to process
    title(['# Im.: ', num2str((r)),'; Proc. Im. #: ', num2str((i)),'; # Rasterp.:',num2str(row*col), '; Est. Time [s] ', num2str(round(estimatedtime)), ';  Elapsed Time [s] ', num2str(round(sum(time)))]);    % plot a title onto the image
    drawnow
